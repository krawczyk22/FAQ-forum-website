...
describe('add question', () => {
	test('login one user', async done => {
        ...
        await page.type('input[name=title]', 'sampleTitle')
		await page.type('input[name=description]', 'sample description')
		await page.click('input[type=submit]')
		await page.waitForSelector('h1')
		await page.waitFor(1000) // sometimes you need a second delay
        ...
        expect(items[1]).toBe('sampleTitle')
        expect(items[2]).toBe('test')
        expect(items[3]).toBe('sample description')
